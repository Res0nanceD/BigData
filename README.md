...
## 0. Создание пользователя для работы с кластером

Для повышения безопасности рекомендуется выполнять установку и запуск сервисов от обычного пользователя, а не от root. Создадим пользователя **hadoopuser** с паролем **Ske8High!**.

На каждом узле (или по крайней мере на тех, где будут запускаться сервисы) выполните следующие команды от имени root или через sudo:
...
## 2. Обеспечение SSH-взаимодействия
...
### 2.3. Распространение публичного ключа  
